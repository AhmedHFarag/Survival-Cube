...
public class Improved_Tracer_Rocket : MonoBehaviour {
    ...
    void Start (){
		...
		TargetObject = Shooter.GetComponent<RocketLauncher>().TargetObject;
		//this is used to show how to print what is happening to the console
		SelectedObject = Shooter.GetComponent<RocketLauncher>().SelectedObject;
        RL = FindObjectOfType<RocketLauncher>();
	}
	void Update (){
        //if (RL.enemies.Length < 1)
        //{
		...
		if (Shooter != null) {
            ...
            if (TargetObject != null)
            {
                ...
                RL.enemies = GameObject.FindGameObjectsWithTag("Enemy");
            }
            ...
            if (Timer > TimeTillExpire)
            {
                ...
                //Die = true;
                // boom=Instantiate(Explosion, transform.position, transform.rotation) as GameObject;
            ...
            CalculatedDistance = Vector3.Distance (gameObject.transform.position, Target);
				//move the tracer
						transform.Translate (0, 0, Speed / 50);
